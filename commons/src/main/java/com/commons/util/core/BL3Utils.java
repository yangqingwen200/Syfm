 ...
 * @since 2013-07-15
 */
public class BL3Utils {

	private static Log log = LogFactory.getLog(BL3Utils.class);
	...
	public static boolean isTheStyle(String pStr, String pStyle) {
		for (int i = 0; i < pStr.length(); i++) {
			...
			if (pStyle.equals(BL3Constants.S_STYLE_N)) {
				if (!Character.isDigit(c))
					return false;
			} else if (pStyle.equals(BL3Constants.S_STYLE_L)) {
				if (!Character.isLetter(c))
					return false;
			} else if (pStyle.equals(BL3Constants.S_STYLE_NL)) {
	 ...
	 *            Dto目标对象
	 */
	public static void copyPropFromBean2Dto(Object pFromObj, com.commons.util.core.Dto pToDto) {
		if (pToDto != null) {
			try {
	...
	public static Timestamp getCurrentTimestamp() {
		Object obj = com.commons.util.core.TypeCaseHelper.convert(getCurrentTime(), "Timestamp", "yyyy-MM-dd HH:mm:ss");
		if (obj != null)
	...
	public static String getGBK(String pString) {
		...
		try {
			...
			log.error(BL3Constants.Exception_Head + "不支持的字符集编码");
			e.printStackTrace();
		}
		return pString;
	}
	 ...
	 * @return
	 */
	public static com.commons.util.core.Dto iterateSetChildren(List list, com.commons.util.core.Dto parent) {
		Iterator<com.commons.util.core.Dto> iterator = list.iterator();
		while (iterator.hasNext()) {
			com.commons.util.core.Dto next = iterator.next();
			if (next.getAsString("parentid").equals(parent.getAsString("id"))) {
				if (parent.getAsList("children") == null) {
					parent.put("children", new ArrayList<com.commons.util.core.Dto>());
				}
	...
	/**
	 * 将dto中为'null',或者为空字符串的统一设置成null
	 * 
	 * @param dto
	 */
	public static void handleNullStr(com.commons.util.core.Dto dto) {
		Iterator ir = dto.keySet().iterator();
		while (ir.hasNext()) {
			String key = (String) ir.next();
			String v = dto.getAsString(key);
			if ("null".equals(v) || BL3Utils.isEmpty(v)) {
				dto.put(key, null);
			}
		}
	}
	 ...
	 * @param map
	 */
	public static void mapRemoveNull(Map map) {
		if (map != null) {
	 ...
	 * 2016年4月6日
	 */
	public static String getErrorMessage(int level) {
		StackTraceElement[] stackTrace = Thread.currentThread().getStackTrace();
		//stackTrace[1] 为当前类和方法: BL3Utils  getErrorMessage
		//stackTrace[2] 为调用者的类和方法: 假如A类的a方法, a方法调用 BL3Utils类getClassAndMethodWeb方法, 则返回 A类 a方法名
		//stackTrace[3] 为调用者_调用者的类和方法: 假如B类b方法调用A类的a方法, a方法调用 BL3Utils类getClassAndMethodWeb方法, 则返回 B类 b方法名
		return stackTrace[level].getClassName() + ", " + stackTrace[level].getMethodName() + "(), due to:";
	}
}
